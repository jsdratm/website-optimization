...
// The max # of pizzas to show on screen
var PIZZA_COUNT = 40;  // JD - Made this a variable so it is easier to find
...
var resizePizzas = function(size) { 
  ...
  // JD - Move this function outside of the loop
  ...
  function determineDx (oldwidth, size) {   // JD - pass width into function instead of element
  ...
  function changePizzaSizes(size) {
	var pizzaContainer = document.getElementsByClassName("randomPizzaContainer"); // JD - Only do the DOM selection once
	
	if (pizzaContainer.length > 0) {
	  // Only calculate the width once since it is the same for each pizza
	  var offsetwidth = pizzaContainer[0].offsetWidth; // JD - only access this property once
	  var dx = determineDx(offsetwidth, size); // JD - only access this property once
	  var newwidth = (offsetwidth + dx) + 'px'; // JD - only access this property once
	  
	  // Loop through the pizza container and set the new width values
      for (var i = 0; i < pizzaContainer.length; i++) {
		var pizzaContainerItem = pizzaContainer[i]; 
  ...
  // JD - Move this outside of the loop
...
var pizzasDiv = document.getElementById("randomPizzas"); // JD - Moved this out of the loop
for (var i = 2; i < 100; i++) {
...
function updatePositions() {
  ...
  var scrollTopFraction = document.body.scrollTop / 1250;  // JD - Moved this calculation out of the loop, seems to have a significant performance impact
  for (var i = 0; i < PIZZA_COUNT; i++) {
    var scaledPhase = Math.sin(scrollTopFraction + (i % 5)) * 100;  // Moved the scaling multiplication to this line
...
document.addEventListener('DOMContentLoaded', function() {
  var cols = 8;
  var s = 256;
  for (var i = 0; i < PIZZA_COUNT; i++) { // JD - 200 was unnecessarily high and 40 seems to be sufficient.  This seemed to have the biggest impact on page performance
